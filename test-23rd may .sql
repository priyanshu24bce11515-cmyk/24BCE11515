-- tables are created in MYSQL 8.0 Command Line Client
show databases;
create database vitcourse;
...
select name, salary from employees  where salary > (select avg(salary) from employees where dept_id = (select dept_id from departments where dept_name='IT')
...
SELECT dept_id, COUNT(*) AS total_emp, AVG(salary) AS avg_salary FROM Employees GROUP BY dept_id HAVING COUNT(*) > 2 AND AVG(salary) > 70000;
...
SELECT e.name FROM Employees e JOIN Jobs j ON e.job_id = j.job_id JOIN Departments d ON e.dept_id = d.dept_id WHERE j.job_title = 'Developer'
...
SELECT j.job_title, MAX(e.salary) AS max_salary FROM Employees e JOIN Jobs j ON e.job_id = j.job_id GROUP BY j.job_title HAVING MAX(e.salary) < 100000;

--10.[SUBQUERY] Find the names of employees who work in the same department as 'David'.
SELECT name FROM Employees WHERE dept_id = ( SELECT dept_id FROM Employees WHERE name = 'David');
...
SELECT name, salary, hire_date FROM Employees WHERE hire_date BETWEEN '2018-01-01' AND '2020-12-31' AND salary > 60000;
...
SELECT j.job_title, e.salary FROM Employees e JOIN Jobs j ON e.job_id = j.job_id WHERE e.emp_id = 504;

--15.[GROUP BY] For each manager (by ID), find the minimum salary of the people reporting to them.
SELECT manager_id, MIN(salary) AS min_salary FROM Employees WHERE manager_id IS NOT NULL GROUP BY manager_id;
